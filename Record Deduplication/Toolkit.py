...
def createGeonameDicts(refPath):
    ...
    for line in open(refPath):
        ...
        if 'State' in containedin:
            states.add(containedin['State'])
            # print(containedin['State'])
        if 'Country' in containedin:
            countries.add(containedin['Country'])
        if jsonobj['a'] == 'City':
            citites.add(jsonobj['name'])

    return {'city': {'names': citites, 'priors': [0 for x in citites]},
            'state': {'names': states, 'priors': [0 for x in states]},
            'country': {'names': countries, 'priors': [0 for x in countries]}}
