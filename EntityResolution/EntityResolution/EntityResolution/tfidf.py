...
def cosineSimilarity(vec, refvec):
    return vec.dot(refvec)/(refvec.dot(refvec)*vec.dot(vec))
...
def generateCandidates(query, refset, revindex):
    ...
    for xx in candidates:
        result.append(Row(uri=xx.label, score=float(query.features.dot(xx.features)), value=query.value))
    return sorted(result, reverse=True, key=lambda k: k.score)[:20]
    # return {'uri': query.label, 'value': query_str.strip(), 'candidates': [x for x in [{ 'score': query.features.dot(xx.features)/(xx.features.dot(xx.features)*query.features.dot(query.features)),
    #                                                                                     'uri': xx.label,
    #                                                                                     'value': xx.value } for xx in refset] if x['score'] > 0.0][:10]}
    # refset.map(lambda x: query.features.dot(x.features))

# def generateCandidates(queries, refset, revindex):
#     print("here!")
...
#         for x in query.words:
#             query_str += x + " "
#         queries.map(lambda x: query.features.dot(x.features))
        # yield {'uri': query.label, 'value': query_str.strip(), 'candidates': sorted(filter(lambda k: k['score'] > 0.0, [{'score': cosineSimilarity(query.features, xx.features),
        #                                                                                     'uri': xx.label,
        #                                                                                     'value': xx.value } for xx in candidates]), reverse=True, key=lambda k: k['score'])[:10]}
        # yield {'uri': query.label, 'value': query_str.strip(), 'candidates': [x for x in [{'score': cosineSimilarity(query.features, xx.features),
        #                                                                                     'uri': xx.label,
        #                                                                                     'value': xx.value } for xx in candidates] if x['score'] > 0.0][:10]}
        # yield {'uri': query.label, 'value': query_str.strip(), 'candidates': [x for x in [{'score': query.features.dot(xx.features)/(xx.features.dot(xx.features)*query.features.dot(query.features)),
        #                                                                                     'uri': xx.label,
        #                                                                                     'value': xx.value } for xx in refset] if x['score'] > 0.0][:10]}
...
#     print("here!")
#     for query in queries:
#         print("HEREWEARE!! " + str(len(refset)) + " " + str(query.value))
#         query_str = ""
#         for x in query.words:
#             query_str += x + " "
#         # yield {'uri': query.label, 'value': query_str.strip(), 'candidates': sorted(filter(lambda k: k['score'] > 0.0, [{'score': cosineSimilarity(query.features, xx.features),
...
class TFIDFMatching:
    def __init__(self):
        ...
        self.qSetPath = "/Users/majid/Dropbox/ISI/dig-entity-resolution/ht-sample-locations.csv.reduced"
        self.outPath = "/Users/majid/Dropbox/ISI/dig-entity-resolution/ht-sample-locations.tfidf.json"
    ...
    def parseInputLineJson(self, x):
        ...
        words = []
        value = str(jobject['name'])
        words += str(jobject['name']).strip().split()
    ...
    def readRef2RDD(self, sparkContext):
        ...
        # res.rdd.saveAsTextFile(self.outPath)
